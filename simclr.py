...
weight_decay = 10e-6
epochs = 100
load_weights_path = 'D:\\Projekte\\MSE\\MSE_SimCLR\\runs\\Apr02_21-57-29_LAPTOP-T1N7HK2E'
...
class SimCLR:
    ...
    def train(self):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(adam_optimizer, T_max=len(train_loader), eta_min=0,
                                                               last_epoch=-1)
        ...
        for epoch_count in range(epochs):
            ...
            self.exec_validate_steps(epoch_count, model, res_net, valid_loader)
            if epoch_count >= 10:
                scheduler.step()  # warmup
            self.tensorboard_writer.add_scalar('cosine annealing lr decay', scheduler.get_lr()[0],
                                               global_step=self.number_of_training_steps)

    def exec_training_steps(self, adam_optimizer, res_net, train_loader):
        # for each image: load both augmented views
        for (xi, xj), _ in train_loader:
            adam_optimizer.zero_grad()
            xi = xi.to(self.processor)
            xj = xj.to(self.processor)
            loss = self.step(res_net, xi, xj)  # execute a training step
    ...
    def step(self, res_net, xi, xj):
        # extract the representation vectors from the augmented views
        hi, zi = res_net(xi)
        hj, zj = res_net(xj)
        # normalize
    ...
    def exec_validate_steps(self, epoch_counter, model_folder, res_net, valid_loader):
        if epoch_counter > 0:
